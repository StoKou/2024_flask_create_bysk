...
def build_preprocessed_eeg_dataset_CNN_test(folder_path):
    '''
    预处理后的 EEG 数据维度为 62 * N，其中62为 channel 数量， N 为采样点个数（已下采样到200 Hz）
    此函数将预处理后的 EEG 信号转化为 CNN 网络所对应的数据格式，即 62 * 200 的二维输入（每 1s 的信号作为一个样本）,区分开不同 trial 的数据
    :param folder_path: Preprocessed_EEG 文件夹对应的路径
    :return feature_vector_dict: 样本的特征向量，key 为被试名字，val 为该被试对应的特征向量的 list，方便 subject-independent 的测试
    '''
    feature_vector_dict = {}

    try:
        # 加载单个 mat 文件
        all_trials_dict=scio.loadmat(folder_path, verify_compressed_data_integrity=False)
        experiment_name=os.path.basename(folder_path).split('.')[0]
        feature_vector_list=[]
        ...
        # 将特征向量列表转换为 NumPy 数组
        feature_vector_dict[experiment_name]=np.asarray(feature_vector_list)

    except FileNotFoundError as e:
        print('加载数据时出错: {}'.format(e))

    return feature_vector_dict
def subject_independent_data_split_test(feature_vector_dict):
    '''
        使用 subject_independent 的方式做数据切分
    :param feature_vector_dict: build_preprocessed_eeg_dataset_CNN 函数返回的 feature_vector_dict
    '''
    test_feature = []
    for trial in feature_vector_dict.keys():
        test_feature.extend(feature_vector_dict[trial])

    return test_feature

class RawEEGDataset_test(torch.utils.data.Dataset):
    def __init__(self, feature_list,  desire_shape):
        self.feature_list = feature_list
        self.desire_shape = desire_shape
